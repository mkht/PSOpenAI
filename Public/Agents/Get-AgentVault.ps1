function Get-AgentVault {
    ...
    param(
        [Parameter(ParameterSetName='Get',Mandatory,Position=0,ValueFromPipelineByPropertyName)] [Alias('vault_id')] [string][UrlEncodeTransformation()]$VaultId,
        [Parameter(ParameterSetName='List')] [ValidateSet('active','archived')] [string[]]$Status,
    ...
    process {
        if($PSCmdlet.ParameterSetName -eq 'Get'){Invoke-AgentApiRequest -EndpointName Agent.Vaults -Path $VaultId -Method Get -Parameters $PSBoundParameters -TypeName PSOpenAI.Agent.Vault}
        else{Invoke-AgentApiRequest -EndpointName Agent.Vaults -Method Get -Parameters $PSBoundParameters -Query ([ordered]@{status=$Status;limit=$Limit;after=$After;order=$Order}) -All:$All -TypeName PSOpenAI.Agent.Vault}
    }
}
