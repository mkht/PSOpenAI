function Get-AgentSessionTurn {
    ...
    process {
        $Base=if($SubagentId){"$SessionId/subagents/$SubagentId/turns"}else{"$SessionId/turns"}; $Path=if($TurnId){"$Base/$TurnId"}else{$Base}
        $Query=if($TurnId){$null}else{[ordered]@{limit=$Limit;after=$After;order=$Order}}
        Invoke-AgentApiRequest -EndpointName Agent.Sessions -Path $Path -Method Get -Parameters $PSBoundParameters -Query $Query -All:($All -and -not $TurnId) -TypeName PSOpenAI.Agent.Session.Turn
    }
}
