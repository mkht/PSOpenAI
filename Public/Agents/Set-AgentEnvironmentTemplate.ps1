function Set-AgentEnvironmentTemplate {
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess($EnvironmentTemplateId, 'Update agent environment template')) {
            Invoke-AgentApiRequest -EndpointName 'Agent.Environments' -Path "templates/$EnvironmentTemplateId" -Method Post -Parameters $PSBoundParameters -Body $PostBody -TypeName 'PSOpenAI.Agent.EnvironmentTemplate'
        }
    }
}
