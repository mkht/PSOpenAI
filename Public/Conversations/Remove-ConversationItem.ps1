function Remove-ConversationItem {
    [CmdletBinding()]
    [OutputType([void])]
    param (
    ...
    process {
        ...
        $UriBuilder.Path += "/$ConversationId/items/$ItemId"
        ...
        if ($Response.deleted) {
            Write-Verbose ('The conversation item with id "{0}" has been deleted.' -f $Response.id)
        }
        #endregion
    }
