function Invoke-AgentApiRequest {
    ...
    $OpenAIParameter = Get-OpenAIAPIParameter `
    ...
    do {
        ...
        if ($Stream) {
            Invoke-OpenAIHttpRequest -Stream @Request |
                ...
                ForEach-Object {
                    try {
                        ...
                        ParseAgentApiObject `
                            -InputObject $Event `
                            -TypeName 'PSOpenAI.Agent.Session.Event'
                    }
                    catch {
        ...
        if ($null -eq $Response -or -not [string]::IsNullOrWhiteSpace($OutFile)) {
            return
        }
        if ($Binary) {
        ...
        try {
            $ResponseObject = $Response | ConvertFrom-Json -ErrorAction Stop
        }
        catch {
            Write-Error -Exception $_.Exception
            return
        ...
        if ($ResponseObject.object -eq 'list' -and $null -ne $ResponseObject.data) {
            $Objects = @($ResponseObject.data)
        }
        else {
        ...
        foreach ($Object in $Objects) {
            ParseAgentApiObject -InputObject $Object -TypeName $TypeName
        }

        $HasMore = $All -and
            [bool]$ResponseObject.has_more -and
            -not [string]::IsNullOrWhiteSpace([string]$ResponseObject.last_id)
        if ($HasMore) {
            $After = $ResponseObject.last_id
        }
    } while ($HasMore)
}
