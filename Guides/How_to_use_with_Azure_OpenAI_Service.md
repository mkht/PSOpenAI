...
The Azure AI Online smoke tests currently cover chat completions and Responses with `gpt-5.6-luna`, audio input/output with `gpt-audio-mini`, and image generation with `gpt-image-2.5-flare`. Model deployment names can be overridden with `AZURE_CHAT_MODEL`, `AZURE_AUDIO_MODEL`, and `AZURE_IMAGE_MODEL`; the tests connect only with `AZURE_OPENAI_ENDPOINT` and `AZURE_OPENAI_API_KEY`. In the test resource, `gpt-transcribe` returned 404 for a v1 transcription request, including with `api-version=preview`, so transcription is excluded from this resource's smoke-test set. This result does not establish that transcription is unsupported across Azure v1; Microsoft's [media API reference](https://learn.microsoft.com/en-us/azure/foundry/openai/reference-preview-latest) documents preview media operations, and availability depends on the resource and deployment.
